...
- (void)handlePanGesture:(UIPanGestureRecognizer *)recognizer{
    ...
    switch (recState) {
        ...
        case UIGestureRecognizerStateEnded:{
            CGPoint stopPoint = CGPointMake(0, JobsSCREEN_HEIGHT / 2.0);
            if (recognizer.view.center.x < JobsSCREEN_WIDTH / 2.0) {
                if (recognizer.view.center.y <= JobsSCREEN_HEIGHT/2.0) {
                    //左上
                    ...
                    //左下
                    if (recognizer.view.center.x  >= JobsSCREEN_HEIGHT - recognizer.view.center.y) {
                        stopPoint = CGPointMake(recognizer.view.center.x,
                                                JobsSCREEN_HEIGHT - self.width/2.0);
                    }else{
                        stopPoint = CGPointMake(self.width / 2.0,
                                                recognizer.view.center.y);
                    }
                ...
                if (recognizer.view.center.y <= JobsSCREEN_HEIGHT/2.0) {
                    //右上
                    if (JobsSCREEN_WIDTH - recognizer.view.center.x  >= recognizer.view.center.y) {
                        stopPoint = CGPointMake(recognizer.view.center.x,
                                                self.width/2.0);
                    }else{
                        stopPoint = CGPointMake(JobsSCREEN_WIDTH - self.width/2.0,
                                                recognizer.view.center.y);
                    }
                }else{
                    //右下
                    if (JobsSCREEN_WIDTH - recognizer.view.center.x  >= JobsSCREEN_HEIGHT - recognizer.view.center.y) {
                        stopPoint = CGPointMake(recognizer.view.center.x,
                                                JobsSCREEN_HEIGHT - self.width/2.0);
                    }else{
                        stopPoint = CGPointMake(JobsSCREEN_WIDTH - self.width/2.0,
            ...
            //如果按钮超出屏幕边缘
            if (stopPoint.y + self.width + 40 >= JobsSCREEN_HEIGHT) {
                stopPoint = CGPointMake(stopPoint.x, JobsSCREEN_HEIGHT - self.width / 2.0 - 49);
            ...
            if (stopPoint.x + self.width / 2.0 >= JobsSCREEN_WIDTH) {
                stopPoint = CGPointMake(JobsSCREEN_WIDTH - self.width/2.0, stopPoint.y);
            }
